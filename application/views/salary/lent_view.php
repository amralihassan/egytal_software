...
<div class="container">
	...
	<div class="w3-bar w3-dark-grey">
		<button class="w3-bar-item w3-button tablink w3-amber pull-right"  id="btn_display"  onclick="openCity(event,'all')">سلف الموظفين</button>
	    <button class="w3-bar-item w3-button tablink  pull-right" onclick="openCity(event,'new')">تسجيل سلفة جديد</button>
	</div>
  	<!-- display all  -->
		...
		<div class="row">
			<div class="col-md-12">
				<form action="" method="post"  id="form_delete" name="frm">
					<a href="#" class="btn btn-danger" onclick="delete_lent();">حذف سلفة</a>	
					<a href="#" class="btn btn-info" id="print_page">طباعة</a>	
					<br><br>
					<!-- tables -->
					<div class="table table-responsive" id="lent_table"></div>
					<div align="center" id="lent_pagination_link"></div> 
				</form>				
			</div>
		</div>			
		<br>
...
<script type="text/javascript">
	...
	function getTotal()
	{
	    ...
	    $.ajax({
	        type:'ajax',
	        url:'<?php echo base_url() ?>Employee/get_total_lent',
			method:'get',
		...
		$.ajax({
			url:'<?php echo base_url() ?>Employee/pagination_search_total_lent/' + 1,
			method:'get',
			data:{fDate:fromDate,tDate:toDate},         
			dataType:'json',
			success:function(data){
				$('#lent_table').html(data.lent_table);
				$('#lent_pagination_link').html(data.lent_pagination_link);
			}
		});	    	
	}	
	$('#print_page').click(function(){
		$('#lent_table').printThis({
		...
		if (data == "") {alert("لم يتم تحديد اي سجلات للحذف");return;}
  		...
  		$('#btnDelete').click(function(){
			$.ajax({
				type:'ajax',
				method:'post',
				url:'<?php echo base_url() ?>Employee/Delete_more_one_lent',
				data:data,
				async:false,
				dataType:'json',
	...
	function searchData(page) 
	{
		...
		$.ajax({
			url:'<?php echo base_url() ?>Employee/pagination_search_lent/' + page,
			method:'get',
	...
	//update
	function update_lent(data)
	{
		var id = data;
		var employeeid = '';
		$('#updateModel').modal('show');
  		$('#updateModel').find('.modal-title').text('تعديل');
  		$('#updateform').attr('action','<?php echo base_url() ?>Employee/update_lent');
  		$.ajax({
  			type:'ajax',
  			method:'get',
  			url:'<?php echo base_url() ?>Employee/getdatabyid_lent',
  			data:{id:id},
  			async:false,
  			dataType:'json',
  			success:function(data){
	...
	$('#btn_save').click(function(){
		var url = '<?php echo base_url() ?>Employee/add_lent';
