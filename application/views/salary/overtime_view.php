...
<div class="container">
	...
	<div class="w3-bar w3-dark-grey">
		<button class="w3-bar-item w3-button tablink w3-amber pull-right"  id="btn_display"  onclick="openCity(event,'all')">الإضافي</button>
	    <button class="w3-bar-item w3-button tablink  pull-right" onclick="openCity(event,'new')">تسجيل إضافي</button>
	</div>
		...
		<div class="row">
			<div class="col-md-12">
				<form action="" method="post"  id="form_delete" name="frm">
					<a href="#" class="btn btn-danger" onclick="delete_overtime();">حذف إضافي</a>	
					<a href="#" class="btn btn-info" id="print_page">طباعة</a>			
					<br><br>	
					<!-- tables -->
					<div class="table table-responsive" id="overtime_table"></div>
					<div align="center" id="overtime_pagination_link"></div> 
				</form>				
			</div>
		</div>
		<br>
...
<script type="text/javascript">
	...
	function getTotal()
	{
	    ...
	    $.ajax({
	        type:'ajax',
	        url:'<?php echo base_url() ?>Employee/get_total_overtime',
			method:'get',
		...
		$.ajax({
			url:'<?php echo base_url() ?>Employee/pagination_search_total_overtime/' + 1,
			method:'get',
			data:{fDate:fromDate,tDate:toDate},         
			dataType:'json',
			success:function(data){
				$('#overtime_table').html(data.overtime_table);
				$('#overtime_pagination_link').html(data.overtime_pagination_link);
			}
		});	    	
	}	

	$('#print_page').click(function(){
		$('#overtime_table').printThis({
		...
		if (data == "") {alert("لم يتم تحديد اي سجلات للحذف");return;}
  		...
  		$('#btnDelete').click(function(){
			$.ajax({
				type:'ajax',
				method:'post',
				url:'<?php echo base_url() ?>Employee/Delete_more_one_overtime',
				data:data,
				async:false,
				dataType:'json',
	...
	function searchData(page) 
	{
		...
		$.ajax({
			url:'<?php echo base_url() ?>Employee/pagination_search_overtime/' + page,
			method:'get',
	...
	//update
	function update_overtime(data)
	{
		var id = data;
		var employeeid = '';
		$('#updateModel').modal('show');
  		$('#updateModel').find('.modal-title').text('تعديل');
  		$('#updateform').attr('action','<?php echo base_url() ?>Employee/update_overtime');
  		$.ajax({
  			type:'ajax',
  			method:'get',
  			url:'<?php echo base_url() ?>Employee/getdatabyid_overtime',
  			data:{id:id},
  			async:false,
  			dataType:'json',
  			success:function(data){
	...
	$('#btn_save').click(function(){
		var url = '<?php echo base_url() ?>Employee/add_overtime';
