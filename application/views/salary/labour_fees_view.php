...
<div class="container">
	<h4>مصاريف العمال</h4><hr>
	  <ol class="breadcrumb">
	    ...
	    <li class="active">مصاريف العمال</li>
	  </ol>
	<!-- alert -->
	<div class="row">
		<div class="col-md-12">
	...
	<div class="w3-bar w3-dark-grey">
		<button class="w3-bar-item w3-button tablink w3-amber pull-right"  id="btn_display"  onclick="openCity(event,'all')">مصاريف الموظفين</button>
	    <button class="w3-bar-item w3-button tablink  pull-right" onclick="openCity(event,'new')">تسجيل مصروف جديد</button>
	</div>
  	<!-- display all -->
		...
		<div class="row">
			<div class="col-md-12">
				<form action="" method="post"  id="form_delete" name="frm">
					<a href="#" class="btn btn-danger" onclick="delete_fees();">حذف مصروف</a>	
					<a href="#" class="btn btn-info" id="print_page">طباعة</a>	
					<br><br>
					<!-- tables -->
					<div class="table table-responsive" id="fees_table"></div>
					<div align="center" id="fees_pagination_link"></div> 
				</form>				
			</div>
		</div>
	</div>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="post" action="" id="updateform">
				    ...
				    <select class="w3-select w3-border" name="emp_full_name_update" id="emp_fullname_id">
				        <option value="">اختار اسم الموظف</option> 	
				    </select>	                    
...
<script type="text/javascript">
	...
	function getTotal()
	{
	    ...
	    $.ajax({
	        type:'ajax',
	        url:'<?php echo base_url() ?>Employee/get_total_fees',
			method:'get',
		...
		$.ajax({
			url:'<?php echo base_url() ?>Employee/pagination_search_total_fees/' + 1,
			method:'get',
			data:{fDate:fromDate,tDate:toDate},         
			dataType:'json',
			success:function(data){
				$('#fees_table').html(data.fees_table);
				$('#fees_pagination_link').html(data.fees_pagination_link);
			}
		});	    	
	}
	// get salary period
	...
	$('#print_page').click(function(){
		$('#fees_table').printThis({
	        ...
	        pageTitle: "مصاريف الموظفين",              // add title to print page
	        removeInline: false,        // remove inline styles from print elements
	        removeInlineSelector: "*",  // custom selectors to filter inline styles. removeInline must be true
	        printDelay: 333,            // variable print delay
	        header: "<h3 class='w3-center'>مصاريف الموظفين</h3>",               // prefix to html
		...
		if (data == "") {alert("لم يتم تحديد اي سجلات للحذف");return;}
  		...
  		$('#btnDelete').click(function(){
			$.ajax({
				type:'ajax',
				method:'post',
				url:'<?php echo base_url() ?>Employee/Delete_more_one_fees',
				data:data,
				async:false,
				dataType:'json',
	...
	function searchData(page) 
	{
		...
		$.ajax({
			url:'<?php echo base_url() ?>Employee/pagination_search_fees/' + page,
			method:'get',
	...
	//update
	function update_fees(data)
	{
		var id = data;
		var employeeid = '';
		$('#updateModel').modal('show');
  		$('#updateModel').find('.modal-title').text('تعديل');
  		$('#updateform').attr('action','<?php echo base_url() ?>Employee/update_fees');
  		$.ajax({
  			type:'ajax',
  			method:'get',
  			url:'<?php echo base_url() ?>Employee/getdatabyid_fees',
  			data:{id:id},
  			async:false,
  			dataType:'json',
  			success:function(data){
	...
	$('#btn_save').click(function(){
		var url = '<?php echo base_url() ?>Employee/add_fees';
